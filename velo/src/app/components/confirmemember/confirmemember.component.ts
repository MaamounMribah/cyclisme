...
@Component({
  selector: 'app-confirmemember',
  templateUrl: './confirmemember.component.html',
  styleUrls: ['./confirmemember.component.css']
})
export class ConfirmememberComponent implements OnInit {
  ...
  confirmercompetition(id:any){
    this.membreequipeservice.confirmermember(id).subscribe((res:any)=>{
      console.log(res)
      Swal.fire("Member equipe confirmée")
      this.GETALLMembreEquipe()
    })
  }
  GETALLMembreEquipe(){
    this.membreequipeservice.getallmembreEquipe().subscribe((res:any)=>{
      this.listmembreEquipe=res.filter((el:any)=>el.equipeFederation.type=='equipe' && el.confirmation=="confirmée")
