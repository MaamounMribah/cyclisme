...
@Component({
  selector: 'app-confirmecoureur',
  templateUrl: './confirmecoureur.component.html',
  styleUrls: ['./confirmecoureur.component.css']
})
export class ConfirmecoureurComponent implements OnInit {
...
//update
  ...
  GETALLCoureur() {
    this.coureurservice.getallcoureur().subscribe((res: any) => {
      this.listcoureur = res.filter((el:any)=>el.confirmation=="confirmée")
...
confirmercoureur(id:any){
  this.coureurservice.confirmercoureur(id).subscribe((res:any)=>{
    console.log(res)
